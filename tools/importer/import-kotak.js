...
// PARSER IMPORTS
import accordionRatesParser from './parsers/accordion-rates.js';
...
import heroHelplineParser from './parsers/hero-helpline.js';
import heroPromoParser from './parsers/hero-promo.js';
import searchParser from './parsers/search.js';
import tabsProfileParser from './parsers/tabs-profile.js';

// TRANSFORMER IMPORTS
import kotakCleanup from './transformers/kotak-cleanup.js';

// PARSER REGISTRY
const parsers = {
  'accordion-rates': accordionRatesParser,
  ...
  'form': formParser,
  'hero-helpline': heroHelplineParser,
  'hero-promo': heroPromoParser,
  'search': searchParser,
  'tabs-profile': tabsProfileParser,
};

const transformers = [kotakCleanup];
...
function selectTemplate(url) {
  ...
  // representativeUrl match
  for (const t of TEMPLATES) {
    if (t.representativeUrl && t.representativeUrl.replace(/\.html$/, ).replace(/\/$/, ) === u) return t;
  }
  // fallback: text-content-page (mostly default content) so unknown URLs still import cleanly
